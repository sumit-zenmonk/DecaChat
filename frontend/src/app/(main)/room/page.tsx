...
import { getMyRooms } from "@/redux/feature/room/room-action";
...
export default function Home() {
    ...
    useEffect(() => {
        const delayDebounceFn = setTimeout(() => {
            setOffset(0);
            dispatch(getMyRooms({ limit, offset: 0, search: searchQuery })).unwrap();
        }, 500);
                    ...
                    >
                        <Box className={styles.roomWrapper}>
                            {filteredRooms && filteredRooms.map((room: Room) => {
                                const isJoined = myrooms ? myrooms.find((joinRoom: Room) => joinRoom.uuid === room.uuid) : null;
                                const memberCount = roomMembersTotalDocuments[room.uuid] || 1;
